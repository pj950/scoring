    ...
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
          ...
          {criteria.map(c => (
            <div key={c.id} className="pb-2">
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                />
                <span
                  key={`${c.id}-${scores[c.id] || 0}`}
                  className="font-bold font-display text-2xl text-cyber-300 w-16 text-center animate-pop-in"
                >
                  {scores[c.id] || 0}
                </span>
              </div>
        ...
        <div className="flex justify-end gap-4 p-6 pt-4 flex-shrink-0 border-t border-slate-700/50">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-600 text-gray-200 rounded hover:bg-slate-500 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 py-2 bg-cyber-600 text-white rounded hover:bg-cyber-500 disabled:bg-slate-700 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors"
          >
            Submit Score
          </button>
        </div>
  ...
  return (
    <div>
      <header className="bg-slate-900/80 backdrop-blur-sm sticky top-0 z-10 border-b border-slate-800 p-4 flex justify-between items-center">
        ...
        <button
          onClick={onLogout}
          className="flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-slate-700 rounded transition-colors"
        >
            ...
            {teams.map(team => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div
                      className="absolute inset-0 border-2 border-cyber-400 animate-trace-border pointer-events-none"
                      style={{
                        clipPath:
                          'polygon(0 0, calc(100% - 20px) 0, 100% 20px, 100% 100%, 0 100%)',
                      }}
